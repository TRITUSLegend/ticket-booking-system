import { PrismaClient, Role, SeatCategory, SeatStatus, EventType } from '@prisma/client';
import bcrypt from 'bcrypt';
import { holdSeats } from './src/modules/seats/seats.service';
import { joinWaitlist, expireOffer } from './src/modules/waitlist/waitlist.service';
...
async function runTests() {
  ...
  const seat2 = await prisma.seat.create({
    data: { layoutId: layout.id, row: 1, column: 2, category: SeatCategory.PREMIUM, label: 'A-2' }
  });
  ...
  try {
    ...
    // Save winner for Test 3
    (global as any).winnerId = winnerId;
  } catch (error) {
  ...
  try {
    ...
    const waitlist = await joinWaitlist(show.id, SeatCategory.PREMIUM, userC.id);
  ...
  try {
    const winnerId = (global as any).winnerId;
