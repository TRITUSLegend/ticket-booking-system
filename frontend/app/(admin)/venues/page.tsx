...
export default function VenuesPage() {
  ...
  if (isLoading) return <div className="p-8 text-center text-white/60">Loading venues...</div>;
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-8">
      ...
      {venues.length === 0 ? (
        <div className="glass-card p-12 text-center">
          <p className="text-white/40">No venues created yet.</p>
        </div>
      ) : (
          ...
          {venues.map((venue) => (
            <div key={venue.id} className="glass-card p-6 flex flex-col justify-between">
              <div>
                ...
                <p className="text-white/60 text-sm mb-4">{venue.address}</p>
              {venue.layouts?.[0] && (
